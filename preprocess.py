...
def process_data():
    ...
    df['Temp_Smooth'] = df['Battery_temp'].rolling(window=10).mean() #Calculating a rolling mean for the temparatures to prevent 0's due to precision 
    df['Temp_Velocity']=df['Temp_Smooth'].diff()
    ...
    print(f"Samples remaining after cleaning: {len(df_clean)}")
    print("-" * 40)
    print("Data Preview (Head):")
    print(df_clean[['Seconds_Run', 'Battery_temp', 'Temp_Smooth', 'Temp_Velocity', 'Stress_Score']].head())
    print("-" * 40)
    #Check for if the phone actually did some work while the data was collected
    max_heat_rate = df_clean['Temp_Velocity'].max()
